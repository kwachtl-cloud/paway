...
export default function BookingsScreen() {
  ...
  return (
    <div className="px-5 pt-6 pb-24 animate-fade-in">
      <h1 className="font-display text-3xl text-text-dark mb-1">{t('bookings')}</h1>
      <p className="text-text-light text-sm mb-6">Your scheduled appointments</p>

      {/* Tabs */}
      <div className="flex bg-white rounded-2xl p-1.5 mb-6 shadow-sm border border-border-subtle">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-text-light">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-beige flex items-center justify-center">
            <Calendar size={26} className="text-text-light" />
          </div>
          <p className="text-base font-medium text-text-dark">{t('noBookingsYet')}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((booking) => (
            <div
              ...
              className="bg-white rounded-3xl p-4 shadow-sm border border-border-subtle active:scale-[0.98] transition-transform cursor-pointer"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between mb-0.5 gap-2">
                    <p className="font-medium text-text-dark text-sm truncate">{booking.providerName}</p>
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold whitespace-nowrap ${statusStyles[booking.status]}`}>
                      {t(booking.status)}
                    </span>
                  </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-border-subtle text-xs text-text-light">
