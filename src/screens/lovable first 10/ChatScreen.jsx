...
export default function ChatScreen() {
  ...
  return (
    <div className="flex flex-col h-screen bg-bg-primary">
      {/* Header */}
      <div className="px-5 pt-5 pb-4 bg-white border-b border-border-subtle flex items-center gap-3">
        <button onClick={goBack} className="w-10 h-10 -ml-2 rounded-full flex items-center justify-center active:scale-95 transition-transform">
          <ArrowLeft size={20} className="text-text-dark" />
        </button>
        <img src={chat.providerImage} alt={chat.providerName} className="w-11 h-11 rounded-full object-cover" />
        <div className="flex-1 min-w-0">
          <p className="font-medium text-text-dark text-sm truncate">{chat.providerName}</p>
          ...
          <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[78%] px-4 py-2.5 ${
              msg.sender === 'user'
                ...
                : 'bg-white text-text-dark rounded-2xl rounded-bl-md shadow-sm border border-border-subtle'
            }`}>
      ...
      <div className="px-4 py-3 bg-white border-t border-border-subtle flex items-center gap-2">
        <input
          ...
          className="flex-1 px-4 py-3 bg-beige rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-brand-green/30"
        />
        <button
          onClick={handleSend}
          disabled={!message.trim()}
          className="w-11 h-11 bg-brand-green text-white rounded-full flex items-center justify-center active:scale-95 transition-transform disabled:opacity-40"
        >
          <Send size={18} />
        </button>
      </div>
